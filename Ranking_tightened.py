...
model.setParam("BarConvTol", 1e-5)   # Tight barrier convergence tolerance
...
######  gain:  gain[i] is a lower bound for the gain of u + gain of ustar for u in [(i-1)/n, i/n)                                       (G_u^T function)
...
######  gain_no_backup: gain_no_backup[i,j] is a lower bound for u in [(i-1)/n, i/n), v in [(j-1)/n, j/n)                               (G_T function for profiles (x_u, x_v, \bot))
...
######### constraints for gain_no_backup, the case where u is matched to v and has no backup b.      (See Fact 11.6, Claim 11.5 And the discretization part in section 11.1)
for u in range(1,n+1):
    for v in range(1,n+1):
        if u<=v:
            for theta_0 in range(0,u+1):
                ...
                ###########     cumulative_of_us contains the integral parts, including gain of u_star in [0,x_v) and the compensation received by u from u^*

                for theta_3 in range(0,theta_0+1):
                    if theta_3 < theta_0:
                        ##########      (Constraint family (1), case i_theta_3 <= i_theta_0 <= i_u <= i_v)
                        model.addConstr(
                            gain_no_backup[u,v]<=cumulative_of_us+
                            max(n-v,0)/n * h[u,theta_0]+
                            max(n-v,0)/n * h[v,u]+
                            max(n-v,0)/n * h[u,theta_3]+
                            max(n-v,0)/n * h[v,theta_3]+
                            theta_3/n * h[u,v]+
                            theta_3/n * h[min(theta_3+1,u),1]+
                            max(theta_0-theta_3,0)/n * h[min(theta_0+1,u),v]+
                            max(n-theta_0,0)/n *  buyer_with_victim[u,v]
                        )
                    else:
                        ##########      (Constraint family (2), case i_theta_3 = i_theta_0 <= i_u <= i_v)
                        model.addConstr(
                            gain_no_backup[u,v]<=cumulative_of_us+
                            max(n-v,0)/n * h[v,u]+
                            max(n-v,0)/n * h[u,theta_3]+
        ...
        if u>=v:
            for theta_0 in range(0,u+1):
                ...
                for theta_3 in range(0,theta_0+1):
                    if theta_3 < theta_0:
                        ##########      (Constraint family (3), case  i_theta_0 , i_v <= i_u  while i_theta_3 < i_theta_0)
                        model.addConstr(
                            gain_no_backup[u,v]<=cumulative_of_us+
                            max(n-max(theta_0,v-1),0)/n * h[v,theta_0]+
                            max(n-max(theta_0,v-1),0)/n * h[v,u]+
                            max(n-max(theta_0,v-1),0)/n * h[v,theta_3]+
                            max(n-max(theta_0,v-1),0)/n * h[v,theta_3]+
                            theta_3/n * h[u,v]+
                            theta_3/n * h[min(theta_3+1,u),1]+
                            max(theta_0-theta_3,0)/n * h[min(theta_0+1,u),v]+
                        ...
                        ##########      (Constraint family (4), case  i_theta_0 , i_v <= i_u  while i_theta_3 < i_theta_0)
                        model.addConstr(
                            gain_no_backup[u,v]<=cumulative_of_us+
                            max(n-max(theta_0,v-1),0)/n * h[v,u]+
                            max(n-max(theta_0,v-1),0)/n * h[v,theta_3]+
                ...
                if theta_0>=u-1: 
                    ###########     (Constraint family (5), case    i_v <= i_u while i_u-1 <= i_theta_0 <= i_u )
                    cumulative_of_us=gp.LinExpr()
                    for us in range(1,theta_0+1):
                        cumulative_of_us += 1/n * g[v,us]
                    model.addConstr(
...
#########     Aggregating the profile constraints (Claim 11.7)
for u in range(1,n+1):
...
###### Integrating over worst case profile distributions (Claim 11.7)

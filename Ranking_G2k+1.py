...
######## This controls the parameter n and k, 
########    n splits the interval (0,1] into n uniform pieces
########    k represent odd girth 2k+1  
...
######  gain:  gain[i] is a lower bound for the gain of u + gain of ustar for u in [(i-1)/n, i/n)                                       (G_u^k function)
######  gain_no_match: gain_no_match[i] is a lower bound for u in [(i-1)/n, i/n) assuming u is unmatched before ustar gets added        (G_k function for profiles (x_u, \bot, \bot))
######  gain_no_backup: gain_no_backup[i,j] is a lower bound for u in [(i-1)/n, i/n), v in [(j-1)/n, j/n)                               (G_k function for profiles (x_u, x_v, \bot))
######                 assuming u is matched to v and does not have a backup before ustar gets added
######  gain_backup: gain_backup[i,j,k] is a lower bound for u in [(i-1)/n, i/n), v in [(j-1)/n, j/n), b in [(k-1)/n, k/n)              (G_k function for profiles (x_u, x_v, x_b))
...
##### function constraints (Claim 10.2)
##### Add monotonicity constraints for g,h and adding constraints that enforces unmatched with <= k copies of compensation always worse than matched            
for u in range(1,n+1):
    for v in range(1,n+1):
...
######### constraints for gain_no_backup, the case where u is matched to v and has no backup b.      (Lemma 10.3)
for u in range(1,n+1):
    for v in range(1,n+1):
        ...
        if u<=v:
            for theta_0 in range(0,u+1):
                ...
                model.addConstr(
                    ...
                    max(n-v,0)/n * h[v,u]+
                    ######## additional gain for u_star
                    max(n-v,0)/n * max(k-2,0) * h[v,theta_0]+
                    theta_0/n * h[u,v]+
                    ######## additional gain for u
...
############ constraints for gain_backup, the case where u is matched to v and has a backup b.  (Lemma 10.4)
for u in range(1,n+1):
    for v in range(1,n+1):
        for b in range(v,n+1):
            if u<=v:
...
#########     Aggregating the profile constraints (Claim 10.5)
for u in range(1,n+1):
...
###### Integrating over worst case profile distributions (Claim 10.5)
